...
describe('JobQueueService', () => {
  ...
  it('addJob adds job to queue and triggers processing', () => {
    const spy = jest
      .spyOn(service as any, 'processNext')
      .mockImplementation(() => {});
  ...
  it('processJob: person not found', async () => {
    ...
    await expect(
      (service as any).processJob({ txId: 1, personId: 2, amount: '10.00' }),
    ).rejects.toThrow();
  });
  ...
  it('processJob: bank not found', async () => {
    dataSource.transaction.mockImplementation(async (cb) => {
      await cb({
        findOne: ({}, opts) =>
          opts.where.id === 2 ? { id: 2, balance: '100.00' } : null,
        save: jest.fn(),
      });
    });
    await expect(
  ...
  it('processJob: success', async () => {
    ...
    expect(txService.updateStatus).toHaveBeenCalledWith(
      1,
      'Success',
      expect.any(Date),
    );
